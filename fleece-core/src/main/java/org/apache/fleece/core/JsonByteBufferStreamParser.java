...
public class JsonByteBufferStreamParser extends JsonBaseStreamParser {
    ...
    /*private static final BufferCache<byte[]> BUFFER_CACHE = new BufferCache<byte[]>(Integer.getInteger(
            "org.apache.fleece.default-char-buffer", 8192)) {
        @Override
    ...
    public JsonByteBufferStreamParser(final Reader reader, final int maxStringLength) {
        ...
        in = new ReaderInputStream(reader, StandardCharsets.UTF_8);
        bb.flip();
    }
    ...
    protected char readNextChar() throws IOException {
        ...
        int avail = in.read(bu, 0, bu.length);
        
        ByteBuffer bbu= ByteBuffer.wrap(bu);
        
        CharBuffer cbb = CharBuffer.allocate(17000);
        
        for(;;){
        CoderResult cr = decoder.decode(bbu, cbb, true);
        System.out.println(cr.isOverflow());
        System.out.println(cr.isUnderflow());
        
        if(cr.isOverflow()) break;
        
        }
       System.out.println("cbb: "+cbb.toString());
        
        
       // char[] cb = new char[8192];

       // int r = read(cb, 0, cb.length);
    ...
    /**
     * {@link InputStream} implementation that reads a character stream from a {@link Reader} and transforms it to a byte stream using a
     * specified charset encoding. The stream
     ...
     * is produced.
     * <p>
     * Since in general it is not possible to predict the number of characters to be read from the {@link Reader} to satisfy a read request
     * on the {@link ReaderInputStream}, all reads from the {@link Reader} are buffered. There is therefore no well defined correlation
     * between the current position of the {@link Reader} and that of the {@link ReaderInputStream}. This also implies that in general there
     * is no need to wrap the underlying {@link Reader} in a {@link java.io.BufferedReader}.
     * <p>
     * {@link ReaderInputStream} implements the inverse transformation of {@link java.io.InputStreamReader}; in the following example,
     * reading from <tt>in2</tt> would return the same byte sequence as reading from <tt>in</tt> (provided that the initial byte sequence is
     * legal with respect to the charset encoding):
     * 
     * <pre>
     * InputStream in = ...
     * Charset cs = ...
     ...
     * </pre>
     * 
     * {@link ReaderInputStream} implements the same transformation as {@link java.io.OutputStreamWriter}, except that the control flow is
     * reversed: both classes transform a character stream into a byte stream, but {@link java.io.OutputStreamWriter} pushes data to the
     * underlying stream, while {@link ReaderInputStream} pulls it from the underlying stream.
     * <p>
     * Note that while there are use cases where there is no alternative to using this class, very often the need to use this class is an
     * indication of a flaw in the design of the code. This class is typically used in situations where an existing API only accepts an
     * {@link InputStream}, but where the most natural way to produce the data is as a character stream, i.e. by providing a {@link Reader}
     * instance. An example of a situation where this problem may appear is when implementing the {@link javax.activation.DataSource}
     * interface from the Java Activation Framework.
     * <p>
     * Given the fact that the {@link Reader} class doesn't provide any way to predict whether the next read operation will block or not, it
     * is not possible to provide a meaningful implementation of the {@link InputStream#available()} method. A call to this method will
     * always return 0. Also, this class doesn't support {@link InputStream#mark(int)}.
     * <p>
