...
public abstract class JsonBaseStreamParser implements JsonChars,
    ...
    private boolean ifConstructingStringValueAdd(char c) throws IOException {
        if (escaped) {
            ...
            if (c == 'u') {
                ...
                if (log) {
                    System.out.println((int) tmp[3] + "/" + (int) tmp[2] + "/"
                            + (int) tmp[1] + "/" + (int) tmp[0]);
                }
        ...
        try {
            while (true) {
                ...
                case KEY_SEPARATOR:
                    ...
                    if (log) {
                        System.out.println(" VAL_IS_VALUE");
                    }
    ...
    private void handleStartObject(final char c) {
        ...
        if (log) {
            System.out.println(" LASIC " + lastSignificantChar);
        }
        ...
        if (lastSignificantChar != QUOTE) {
            ...
            default: // number
                ...
                while (true) {
                    ...
                    if (log) {
                        System.out.println("dotpassed:" + dotpassed
